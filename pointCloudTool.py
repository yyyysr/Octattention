...
PCERRORPATH = "OctAttention-obj/OctAttention-obj/file/pc_error"
TEMPPATH = "temp/data/"
...
def ptread(path, color_format):  # point read
    """
    Load ptcloud
    Returns: coords.
    """
    # pcread()读入PLY或者PCD格式的三维点云--ysr
    if (color_format == 'rgb'):
        pt, color = pcread(path, color_format)
        return pt, color
    pt, _ = pcread(path, color_format)
    return pt


def pcread(path, color_format='rgb'):  # 这里可以读入颜色，默认是带颜色属性读入
    """
    Load ptcloud
    ...
    if path.endswith(".ply"):  # endswith用于判断是否是以.ply后缀结尾
        try:
...
def loadply(filedir, color_format='rgb'):
    ...
    for line in files:
        ...
        try:  # 跳过前面的空行
            x, y, z, = float(wordslist[0]), float(wordslist[1]), float(wordslist[2])  # 按行取出xyz的坐标值，3*n
            if color_format != 'geometry':
                try:
                    r, g, b = float(wordslist[3]), float(wordslist[4]), float(
                        wordslist[5])  # 如果color_format是grb的话，就把颜色也直接读出来,6*n
                except IndexError:
                    color_format = 'geometry'
        except ValueError:
            continue
    ...
    if color_format == 'rgb':
        ...
        # feats = np.expand_dims(np.ones(coords.shape[0]), 1)
        return coords, None  # 几何读取数据直接返回坐标
